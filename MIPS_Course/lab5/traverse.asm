...
check_left_in:
        # visit all left nodes first
        lw      $a0, 4($s1)                     # load left value of left node into a0
        beq     $a0, $zero, print_root_in       # if left node doesn't exist, we print the root
        jal     traverse_tree                   # if node does exist, we call traverse on that node
        
print_root_in:
        beq     $a0, 
        move    $a0, $s1                        # move a0 back to the value
        jal     print_func2                     # print what's stored at a0
        j       check_right_in                  # do the right node stuff

check_right_in:
        lw      $a0, 8($s1)                     # load the node on the right
        beq     $a0, $zero, traverse_done       # if right node doesn't exist, exit recursion
        jal     traverse_tree                   # perform recursion on right node if it exists
