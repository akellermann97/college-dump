...
**
** parameters:
**    line   current input line number
...
static Status add_logentry( int line, int id, int date, int time,
    ...
    while( ercurr &&
           ercurr->date < event->date &&
           ercurr->time <= event->time ) {
...
**
** return:
**    count of logfile records loaded
*/
...
void generate_report( void ) {
    ...
    printf( "Number of employees with log entries:  %d\n", num_logentries );
    ...
    for( entry = logfile; entry; entry = entry->next ) {
        ...
        if( emp == NULL ) {
            ...
            fputs( " hired ", stdout );
            printf( "%4d/%02d/%02d:\n", emp->hire / 10000,
                    (emp->hire % 1000) / 100, emp->hire % 100 );

            // now, iterate through the event list for this employee

            for( event = entry->first; event; event = event->next ) {

                printf( "\t%4d/%02d/%02d", event->date / 10000,
                        (event->date % 1000) / 100, (event->date % 100) );
